...
def main():
    ...
    if config['augmentation']['enabled'] and config['augmentation']['sliding_window']:
        print("Using sliding window augmentation...")
        window_size = config['augmentation']['window_size']
        window_step = config['augmentation']['window_step']
        ...
        for eeg, fnirs, label_info in preprocessed:
            ...
            # Sliding windows on EEG (active period only)
            active_start = config['eeg']['active_start']
            eeg_active = eeg[active_start:, :]
            
            for start_idx in range(0, len(eeg_active) - window_size, window_step):
                eeg_window = eeg_active[start_idx:start_idx + window_size, :]
                # Pad back to original size
                eeg_aug = np.zeros_like(eeg)
                eeg_aug[:window_size, :] = eeg_window
                
                features = feature_extractor.extract(eeg_aug, fnirs)
                X_list.append(features)
                y_list.append(label_info['label'])
    else:
        for eeg, fnirs, label_info in preprocessed:
